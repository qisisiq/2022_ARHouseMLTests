...
namespace MagicLeap.Examples
{
    ...
    /// </summary>
    public class ARDKMeshing : MonoBehaviour
    {
        private static ARDKMeshing _instance;
        public static ARDKMeshing Instance
        {
        ...
        [SerializeField, Space, Tooltip("Ivy seed that shoots into the scene.")]
        ...
        private void Start()
        {
           ...
           // var _arSession = ARSessionFactory.Create();
           // Debug.Log("are location permission permitted?: "+ Permission.HasUserAuthorizedPermission(Permission.FineLocation));
            
           // CreateWayspotStuff();
        }

        /*
        void CreateWayspotStuff()
        {
            ...
            locationService.Start(0.01f, 0.01f);
        ...
        void WayspotControllerStuff()
        {
            var locationService = LocationServiceFactory.Create(_arSession.RuntimeEnvironment);
            // For LocationService, we recommend using a desiredAccuracyInMeters of 0.01f
            // and an updateDistanceInMeters of 0.01f for best results
            locationService.Start(0.01f, 0.01f);
            var _wayspotAnchorController = new WayspotAnchorController(_arSession, locationService);
        ...
        private void LocalizationStateUpdated(LocalizationStateUpdatedArgs args)
        {
            ...
            Debug.Log("localization state updated!: " + args);
        }
        ...
        private void OnTriggerDown(InputAction.CallbackContext callbackContext)
        {
            ...
            if (Physics.Raycast(
                ...
                _controller.gameObject.transform.forward,
                out var hit,
                Mathf.Infinity, 
                LayerMask.NameToLayer("MagicLeapMesh")))
            {
        ...
        {
            GameObject seed;
            seed = Instantiate(_seedPrefab);
            seed.SetActive(true);
            
            float seedsize = Random.Range(MIN_BALL_SIZE, MAX_BALL_SIZE);
            seed.transform.localScale = new Vector3(seedsize, seedsize, seedsize);
            seed.transform.position = _controller.gameObject.transform.position;
            
            Rigidbody rigidBody = seed.GetComponent<Rigidbody>();
            if (rigidBody == null)
            {
                rigidBody = seed.AddComponent<Rigidbody>();
            }
